...
Sur le S25, Android peut ignorer un changement de mode associé à une désactivation. La session prépare donc le mode initial dans une première commande active, puis restaure l’état désactivé exact. Chaque étape conserve avant écriture sa cible et son prédécesseur dans le journal, compatible avec l’ancien format. Une interruption se reprend uniquement depuis un état reconnu ; l’abandon du journal partage le verrou des commandes. Un centre absent à l’arrêt reste inconnu, sans coordonnées inventées.

La recette du 25 septembre 2026 a confirmé par relecture publique le cycle OFF/WINDOW/1 → ON/FULLSCREEN/1,8 → OFF/WINDOW/1 et les commandes historiques ON/WINDOW/1,5 → OFF/WINDOW/1, sans modification des profils. Cette preuve de fonctionnement sur appareil ne démontre ni bénéfice perceptif ni validation commerciale. Les deux contrôles instrumentés dépendant du service ont été remplacés par ces recettes dans le processus normal ; aucune permission Lab n’a été accordée.
