...
export const slugs = [
  'android', 'fonctionnalites', 'tests-visuels', 'profils-intelligents', 'lecteur', 'faq',
...
export const routes: Record<Lang, Record<Slug, string>> = {
  ...
  en: {
    android: 'android', fonctionnalites: 'features', 'tests-visuels': 'visual-tests',
...
export const ui = {
  fr: {
    ...
    android: 'Découvrir la bêta Android', iphone: 'iPhone — bientôt disponible',
  ...
  en: {
    ...
    android: 'Explore the Android beta', iphone: 'iPhone — coming soon',
...
export const pages: Record<Lang, Record<Slug, Page>> = {
  fr: {
    android: { title:'VueConfort pour Android', eyebrow:'Bêta Android', description:'Découvrez la bêta fonctionnelle VueConfort pour Android, actuellement testée sur Galaxy S25 avant sa publication Google Play.', intro:'Bêta fonctionnelle — publication Google Play en préparation.', sections:[] },
    ...
    telechargement: { title:'VueConfort sur votre téléphone', eyebrow:'Téléchargement', description:'Disponibilité de VueConfort pour Android et état du développement de l’application iPhone.', intro:'Les versions officielles seront distribuées par Google Play et l’App Store lorsqu’elles seront prêtes. Aucun téléchargement direct non vérifié n’est proposé.', sections:[
      {title:'Android · bêta fonctionnelle',body:'Version 1.0.0 actuellement testée sur Samsung Galaxy S25. La validation sur Galaxy A53 et la publication Google Play restent à terminer. Découvrez la page Android pour connaître les fonctions, la compatibilité et les limites.'},
...
pages.en = {
  android:{title:'VueConfort for Android',eyebrow:'Android beta',description:'Explore the functional VueConfort Android beta, currently tested on Galaxy S25 before its Google Play release.',intro:'Functional beta — Google Play release in preparation.',sections:[]},
  ...
  telechargement:{title:'VueConfort on your phone',eyebrow:'Download',description:'VueConfort availability for Android and current iPhone development status.',intro:'Official versions will be distributed through Google Play and the App Store when ready. No unverified direct download is offered.',sections:[{title:'Android · functional beta',body:'Version 1.0.0 is currently tested on Samsung Galaxy S25. Galaxy A53 validation and Google Play publication remain to be completed. See the Android page for features, compatibility and limits.'},{title:'iPhone app in development',body:'Planned features: visual tests, profiles, internal reader, text and PDF import, and a share extension. iOS allows no global bubble, unrestricted reading of other apps or global filter.'},{title:'Safe installation',body:'Do not install APKs or profiles from third-party sites. Always verify the official VueConfort publisher and domain.'}]},
...
// Set this single value when the official public Google Play listing exists.
export const androidStoreUrl: string | null = null;

export const androidBeta = {
  fr: {
    status:'Bêta fonctionnelle — publication Google Play en préparation', badge:'Bêta Android',
    facts:[['Version','1.0.0'],['Plateforme','Android'],['Statut','Bêta'],['Traitement','Local'],['Compte','Non requis'],['Publicité','Absente'],['Analytics','Absents']],
    features:[
      ['Assistance au quotidien',['Barre flottante au-dessus des applications','Grossissement Android réel','Commandes + et −','Notification persistante','Tuile de réglages rapides']],
      ['Profils visuels',['Profils personnalisables','Profils automatiques selon l’application','Règles selon l’heure et la luminosité']],
      ['Lecture adaptée',['Extraction du texte accessible uniquement à la demande','Lecteur adapté avec réglages de présentation','Aucune capture d’écran']],
      ['Tests STANDARDIZED_V2',['Landolt C','E directionnel','Contraste sur écran','Grille d’Amsler']]
    ],
    compatibility:['Testé actuellement : Samsung Galaxy S25.','Test prévu ou en cours : Samsung Galaxy A53.','Compatibilité attendue : smartphones Android modernes prenant en charge le service d’accessibilité et le grossissement Android.'],
    dependencies:['version d’Android','personnalisation du constructeur','activation du service d’accessibilité','capacités de grossissement du système','contenu accessible exposé par les autres applications'],
    permissions:['piloter le grossissement Android','afficher la barre flottante','détecter l’application active pour appliquer les profils','extraire le texte accessible uniquement lorsque vous choisissez Lire'],
    protections:['Aucune capture d’écran','Aucun mot de passe enregistré','Aucun contenu transmis','Aucune action automatique de saisie'],
    limits:['Certains contenus ne fournissent aucun texte accessible.','Les jeux, images, vidéos et écrans sécurisés peuvent être incompatibles avec le lecteur.','Les effets optiques avancés ne sont pas appliqués globalement aux pixels des autres applications.','Les tests visuels sont des outils de dépistage et non un diagnostic.'],
    screenshots:['Accueil','Barre flottante dans Chrome','Grossissement','Lecteur','Profils','Tests visuels']
  },
  en: {
    status:'Functional beta — Google Play release in preparation', badge:'Android beta',
    facts:[['Version','1.0.0'],['Platform','Android'],['Status','Beta'],['Processing','On-device'],['Account','Not required'],['Advertising','None'],['Analytics','None']],
    features:[
      ['Everyday assistance',['Floating bar over other apps','Real Android magnification','+ and − controls','Persistent notification','Quick Settings tile']],
      ['Visual profiles',['Customisable profiles','Automatic profiles by app','Time and ambient-light rules']],
      ['Adapted reading',['Accessible text extracted only on request','Adapted reader with presentation settings','No screen capture']],
      ['STANDARDIZED_V2 tests',['Landolt C','Directional E','On-screen contrast','Amsler grid']]
    ],
    compatibility:['Currently tested: Samsung Galaxy S25.','Planned or ongoing test: Samsung Galaxy A53.','Expected compatibility: modern Android smartphones supporting the accessibility service and Android magnification.'],
    dependencies:['Android version','manufacturer customisation','accessibility service activation','system magnification capabilities','accessible content exposed by other apps'],
    permissions:['control Android magnification','display the floating bar','detect the active app for profiles','extract accessible text only when you choose Read'],
    protections:['No screen capture','No password stored','No content transmitted','No automatic typing action'],
    limits:['Some content exposes no accessible text.','Games, images, videos and secure screens may be incompatible with the reader.','Advanced optical effects are not applied globally to pixels from other apps.','Visual tests are screening tools, not a diagnosis.'],
    screenshots:['Home','Floating bar in Chrome','Magnification','Reader','Profiles','Visual tests']
  }
} as const;
